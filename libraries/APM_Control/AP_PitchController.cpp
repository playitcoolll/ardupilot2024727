...
float AP_PitchController::_get_rate_out(float desired_rate, float scaler, bool disable_integrator, float aspeed,bool ground_mode)
{
    ...
    if(_ahrs.get_velocity_NED(Ve)){
        ...
        float boost_az= 2.5;//飞翼不同速度下线化 根轨迹求出的结果
        if(aspeed >= 10){
    ...
    boost_aoa = 1.66;//飞翼不同速度下线化 根轨迹求出的结果
    if(aspeed >= 10){
    ...
    //姿态反馈pid方法一
    float boost_pitch = 0.1;//飞翼冲浪者
    //计算P
    error_my = desired_rate-degrees(rate_y);
    float out_my_p = boost_pitch * error_my;

    //计算I
    float ki_pitch = 0.2;//飞翼冲浪者
    if(!disable_integrator){
        if(underspeed){
            ki_pitch = 0.0;
        }
    ...
    float k_d = 1;
    if(aspeed > 4){
        k_d = 1 + (0.1/10*(aspeed - 4));//
    }
    float k_i = 1;
    if(aspeed > 4){
        k_i = 1 + (0.1/10*(aspeed - 4));//
    }


    float out_my = out_my_p + I_term * k_i + D_term * k_d + out_aoa + out_az;//
